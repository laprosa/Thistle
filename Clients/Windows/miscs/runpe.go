...
// SuspendProcess suspends a running process using its handle
func SuspendProcess(hProcess syscall.Handle) error {
	// Load ntdll.dll
	ntdll := syscall.NewLazyDLL("ntdll.dll")

	// Load the NtSuspendProcess function
	ntSuspendProcess := ntdll.NewProc("NtSuspendProcess")

	// Call NtSuspendProcess
	status, _, _ := ntSuspendProcess.Call(uintptr(hProcess))
	if status != 0 {
		return fmt.Errorf("failed to suspend process. Status: 0x%x", status)
	}

	return nil
}

// ResumeProcess resumes a suspended process using its handle
func ResumeProcess(hProcess syscall.Handle) error {
	// Load ntdll.dll
	ntdll := syscall.NewLazyDLL("ntdll.dll")

	// Load the NtResumeProcess function
	ntResumeProcess := ntdll.NewProc("NtResumeProcess")

	// Call NtResumeProcess
	status, _, _ := ntResumeProcess.Call(uintptr(hProcess))
	if status != 0 {
		return fmt.Errorf("failed to resume process. Status: 0x%x", status)
	}

	return nil
}

// CleanUp closes the process and thread handles
func CleanUp(hProcess, hThread syscall.Handle) {
	syscall.CloseHandle(hProcess)
	syscall.CloseHandle(hThread)
}

// Inject starts the src process and injects the target process.
func Inject(srcPath string, sc []byte, args *string) *syscall.ProcessInformation {
	...
	hProcessID := uintptr(pi.ProcessId)

	Log("Process created. Process: %v, Thread: %v, ProcessID: %v", hProcess, hThread, hProcessID)
